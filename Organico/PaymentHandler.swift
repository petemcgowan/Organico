...
class PaymentHandler: NSObject {
    ...
    func startPayment(groomingItems: GroomingItems, total: Double, completion: @escaping PaymentCompletionHandler) {
        ...
        groomingItems.cartContents.forEach { groomingItem in
            let item = PKPaymentSummaryItem(label: groomingItem.title, amount: NSDecimalNumber(string: "\(groomingItem.price).00"), type: .final)
        ...
        paymentRequest.merchantIdentifier = "merchant.io.buachaillmaith.organico"
        paymentRequest.merchantCapabilities = .capability3DS // A security protocol used to authenticate users
        paymentRequest.countryCode = "IE"
        paymentRequest.currencyCode = "EU"
